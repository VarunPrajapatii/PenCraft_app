...
const PublishHeader = ({
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 bg-white/20 dark:bg-black/30 backdrop-blur-lg shadow-lg">
...
const SaveDraftButton = ({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={`group relative inline-flex items-center rounded-full bg-gray-200 dark:bg-gray-700
    ...
    >
      {DraftLoading ? (
        <svg className="animate-spin h-5 w-5 text-red-500 dark:text-red-400 mr-0 lg:mr-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
      ...
      <span className={`hidden lg:block transition-transform duration-200 group-hover:translate-x-1 group-hover:text-red-700 dark:group-hover:text-red-400`}>
...
const PublishButton = ({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={`group relative inline-flex items-center rounded-full bg-green-200 dark:bg-green-800
                        px-4 py-2 sm:px-3 sm:py-2 md:px-5 md:py-2 text-sm sm:text-base md:text-lg
                        font-semibold text-black dark:text-white shadow transition-all duration-200 cursor-pointer
                        hover:bg-gradient-to-r hover:from-green-400/30 hover:to-blue-400/30
                        dark:hover:from-green-500/30 dark:hover:to-blue-500/30
                        hover:shadow-lg hover:scale-105
                        active:scale-95 active:shadow
    ...
    >
      {PublishLoading ? (
        <svg className="animate-spin h-5 w-5 text-green-600 dark:text-green-400 mr-0 lg:mr-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
          <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
          <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z"></path>
        </svg>
      ) : (
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor"
          className="md:inline-block size-6 lg:mr-2 transition-transform duration-200 group-hover:translate-x-1 group-hover:text-green-700 dark:group-hover:text-green-400">
          <path strokeLinecap="round" strokeLinejoin="round" d="M6 12 3.269 3.125A59.769 59.769 0 0 1 21.485 12 59.768 59.768 0 0 1 3.27 20.875L5.999 12Zm0 0h7.5" />
        </svg>
      )}
      <span className={`hidden lg:block transition-transform duration-200 group-hover:translate-x-1 group_hover:text-green-700 dark:group-hover:text-green-400`}>
